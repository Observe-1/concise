-- Recurring transactions may now apply a percentage of the target's current
-- value instead of a fixed amount. Exactly one of amount_minor / percent is
-- set, selected by amount_type. SQLite cannot modify CHECK constraints, so
...
INSERT INTO recurring_transactions_new (id, user_id, name, asset_id, liability_id,
                                        amount_type, amount_minor, percent, cadence,
                                        next_run_on, last_run_on, active, created_at)
SELECT id, user_id, name, asset_id, liability_id,
       'fixed', amount_minor, NULL, cadence,
       next_run_on, last_run_on, active, created_at
